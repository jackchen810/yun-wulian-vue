<template>
  <div class="table" v-loading="loading2">
    ...
    <el-form :inline="true" class="handle-box">
      <el-form-item>
        <el-button
          ...
          @click="showDialogCreateUser=true"
        >新建账号</el-button>
      </el-form-item>
      <el-form-item label>
    ...
    >
      ...
      <el-table-column prop="user_region" label="部门"></el-table-column>
      ...
      <el-table-column prop="user_status" label="冻结状态" width="120">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.user_status == '1' ? 'warning' : 'success'"
            close-transition
          >{{scope.row.user_status=='1'?'已冻结':'未冻结'}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="渠道类型" width="150">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.user_type == '0' ? 'danger' : 'info'"
            close-transition
          >{{scope.row.user_type == '0'?'超级管理员':'管理员'}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="450">
        <template slot-scope="scope">
          <el-button
            ...
            size="small"
            type="success"
            @click="modifyPassword(scope.row.user_account, scope.row.user_password)"
          >修改密码</el-button>
          <!--<el-button class="btn1" size="small" type="warning" @click="resetPassword(scope.row.user_account)">重置密码</el-button>-->
          <el-button
            class="btn1"
            size="small"
            v-if="scope.row.user_status =='0' && scope.row.user_type =='1'"
            @click="revoke(scope.row.user_account)"
            :type="scope.row.user_status == '1' ? 'warning' : 'danger'"
          >冻结账户</el-button>
          <el-button
            class="btn1"
            size="small"
            v-if="scope.row.user_status =='1' && scope.row.user_type =='1'"
            @click="restore(scope.row.user_account)"
            :type="scope.row.user_status == '1' ? 'warning' : 'danger'"
          >解冻账户</el-button>
          <el-button
            class="btn1"
            size="small"
            v-if="scope.row.user_type =='1'?true:false"
            type="danger"
            @click="deleteAccount(scope.row.user_account)"
          >删除账户</el-button>
          <el-button
            class="btn1"
            size="small"
            v-if="scope.row.user_type =='1'?true:false"
            type="success"
            @click="getOwnProject(scope.row.user_account)"
          >添加项目</el-button>
    ...
    <el-dialog title="修改密码" :visible.sync="showDialogPwd" class="digcont">
      <el-form :model="formP" ref="formP" :rules="rulesP">
        ...
        <el-form-item label="新密码" prop="user_new_password" :label-width="formLabelWidth">
          <el-input
            v-model="formP.user_new_password"
            type="password"
            class="diainp"
            auto-complete="off"
          ></el-input>
        ...
        <el-form-item label="确认密码" prop="user_validate_password" :label-width="formLabelWidth">
          <el-input
            v-model="formP.user_validate_password"
            type="password"
            class="diainp"
            auto-complete="off"
          ></el-input>
    ...
    <el-dialog title="新建账号" :visible.sync="showDialogCreateUser" class="digcont">
      <el-form :model="formA" :rules="rulesA" ref="formA">
        ...
        <el-form-item label="密码" prop="password" :label-width="formLabelWidth">
          <el-input
            v-model="formA.user_password"
            type="password"
            class="diainp"
            auto-complete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="部门" prop="name" :label-width="formLabelWidth">
      ...
      <el-form :model="formPrj" ref="formPrj" :rules="rulesP">
        <el-form-item label>
          <el-checkbox-group v-model="formPrj.select_list">
            <el-checkbox v-for="item in projectList" :key="item" :name="'type'" :label="item"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @click="updateOwnProject(formPrj)"
          v-loading.fullscreen.lock="fullscreenLoading"
        >确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
      ...
      self.$axios.post("api/admin/list", params).then(function (res) {
        ...
        if (res.data.ret_code == 0) {
          ...
          if (params.user_status == undefined) {
            ...
            console.log("params.user_status===" + params.user_status);
            console.log(self.userData)
            var arrobj = []
            res.data.extra.map((item) => {
              let arry = {};
              if ( item.user_status == params.user_status) {
                  arry = item
                
                self.userData.push(arry)
              }
            });
            console.log(self.userData)
            self.pageTotal = self.userData.length;
          }
        } else {
          self.userData = [];
        }
    ...
    changeTab: function () {
      ...
      if (this.radio3 == "all") {
        this.currentPage = 1;
        this.getUsers(params, "all");
      } else {
        this.getUsers({ user_status: this.radio3 }, "status");
      }
    },
    revoke: function (account) {
      //冻结操作
      var self = this;
      var params = {
      ...
      self.loading = true;
      self.$axios.post("api/admin/revoke", params).then(
        function (res) {
          self.loading = false;
          ...
          if (res.data.ret_code == 0) {
            ...
            if (self.radio3 == "all") {
              self.getUsers(
                { page_size: 10, current_page: self.currentPage },
                "all"
              );
            } else {
              self.getUsers(
                {
                  user_status: self.radio3,
                  page_size: 10,
    ...
    restore: function (account) {
      ...
      self.loading = true;
      self.$axios.post("api/admin/restore", params).then(
        function (res) {
          self.loading = false;
          if (res.data.ret_code == 1001) {
            self.$message({ message: res.data.ret_msg, type: "warning" });
    ...
    getOwnProject: function (account) {
      ...
      self.loading = true;
      self.$axios
        .post("api/admin/get/own/project", params)
        .then(function (res) {
          self.loading = false;
          if (res.data.ret_code == 0) {
    ...
    updateOwnProject: function (formPrj) {
      ...
      console.log(
        "[AccountManage] update own project list, user_account: " +
          formPrj.user_account
      );
      console.log(
        "[AccountManage] update own project list, select_list: " +
          formPrj.select_list
      );
      let params = {
      ...
      self.loading = true;
      self.$axios
        .post("api/admin/update/own/project", params)
        .then(function (res) {
          self.loading = false;
          if (res.data.ret_code == 0) {
            self.$message({ message: res.data.ret_msg, type: "success" });
    ...
    deleteAccount: function (account) {
      ...
      this.$confirm("此操作将永久删除该账号, 是否继续?", "提示", {
        ...
        .then(() => {
          self.$axios
            .post("api/admin/delete", { user_account: account })
            .then(function (res) {
              if (res.data.ret_code == 0) {
    ...
    search: function () {
      ...
      self.$axios.post("api/admin/list", params).then(function (res) {
        self.loading = false;
          console.log(res);
          console.log(self.search_word);
            var arrobj = []
            res.data.extra.map((item) => {
              let arry = {};
             
              if (item.user_account == self.search_word || item.user_region == self.search_word) {
                  arry = item
                self.userData.push(arry)
              }
            });
    ...
    savePwdChange: function (formP) {
      ...
      self.$refs[formP].validate(function (valid) {
        if (valid) {
          ...
          self.$axios.post("api/admin/change", params).then(
            function (res) {
              ...
              if (res.data.ret_code == 0) {
                ...
                for (let i = 0; i < self.userData.length; i++) {
                  if (
                    self.userData[i].user_account == self.formP.user_account
                  ) {
                    self.userData[i].user_password =
